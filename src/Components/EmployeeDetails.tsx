...
export default function EmployeeDetails() {
  ...
  const visbleEmployees = employee.slice(startIndex, startIndex + pageSize)
  ...
  const handilemployeeStatus = async (id: number, currentStatus: boolean) => {
    ...
    axios.post(`${url}/employee/status/${id}`, {
  ...
  return (
    <div className="w-5/6  m-auto my-10">
      <TableContainer component={Paper} className="shadow-employee   rounded-[14px]">
        <Table className="h-[70px]">
          <TableHead className="bg-greenAcc h-[70px]  ">
            <TableRow className="grid grid-cols-4 ">
              {colum.map((col) => (
                <TableCell className=" text-white flex items-center [&:nth-child(3)]:pl-[100px] [&:nth-child(2)]:pl-[70px] font-tinos text-[24px] font-bold [&:nth-child(4)]:text-yellowAcc [&:nth-child(4)]:m-auto [&:nth-child(4)]:pl-[45px]" key={col.id}>

                  <FormControlLabel control={<Checkbox style={{ color: '#F6C90E' }} />} label="" />
      ...
      {visbleEmployees.map((user) => (
        <div key={user.id} className="grid gap-x-16 font-roboto text-[22px] grid-cols-4  my-4 bg-white rounded-[14px] shadow-employee h-[70px] items-center px-5">
          <div>
            <FormControlLabel control={<Checkbox style={{ color: '#F6C90E' }} />} label="" />
            <span className="ml-[-20px] font-roboto text-[22px] text-greenD">
              {user.name}
            </span>
          </div>
          <div className="text-yellowAcc text-[18px]">
            {user.email}
          </div>
          <div className="text-[20px] text-greenD ml-[80px] w-[150px]">{user.phone}</div>
          <div className="flex gap-x-3 items-center w-[180px] ms-auto">


            <Switch
              checked={user.status}
              onChange={() => handilemployeeStatus(user.id, user.status)}
              style={{ color: '#F6C90E' }} className=''
            />

            <Link to={`/EditEmployee/${user.id}`}><img src={icon1} /></Link>
            <Link to='' onClick={() => DeleteEmployee(user.id)}> <img src={icon2} /></Link>
          </div>
